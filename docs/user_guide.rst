...
Compare Equal is a package of two function and a subpackage of tests for each function. As the package uses Chimera classes and its attributes, it is essential to have it installed. The package import the Chimera module:
...
Both function return a bolean value (True or False) depending if both molecules are equal not.
...
.. py:function:: CompareMol(mol_1,mol_2)
    ...
    :param mol_1: A Python variable to compare with *mol2*.
    :param mol_2: A Python variable to compare with *mol1*.
    :type mol_1: 'chimera.Molecule' object
    :type mol_2: 'chimera.Molecule' object
    :return: A True or False value.
    :rtype: bolean
    :raises AssertionError: if one of the argument is not a Chimera Molecule object